 ...
 * @author silen
 */
@WebServlet(urlPatterns = {"/answeraquestion"})
public class answeraquestion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String message = "Question Successfully Answered";
        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                ...
                if (!db.updateQuestion(dto)) {
                    ...
                    getServletContext().getRequestDispatcher("/answeraquestion2.jsp").forward(request, response);
                    return;
                }
                request.setAttribute("message", message);
                request.setAttribute("color", color);
                request.setAttribute("id", id_s);
                getServletContext().getRequestDispatcher("/answeraquestion.jsp").forward(request, response);
                return;
